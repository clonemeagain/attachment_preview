...
class AttachmentPreviewPlugin extends Plugin {
	...
	private function checkPermissionsAndRun() {
		...
		if (in_array ( $config->get ( 'attachment-enabled' ), array (
			...
			// Can't go injecting this into every page.. we only want it for the actual ticket pages or KB pages
			// Check URI, OR if not Ticket, check for permission to inject on KB, then see if it is ALSO a KB page.
			if ($this->isTicketsView () || ($config->get ( 'attachment-enabled-kb' ) && $this->isKBView ())) {
	...
	private function findAttachableStuff($html) {
		...
		// deciphering what the admin chose, also determines what method to run for each extension type.
		...
		// Our osTicket attachments are always "links", which HTML refers to via "anchor" or <a> tags. 
		// We can iterate quite quickly through every tag named "a" and check it.
		foreach ( $doc->getElementsByTagName ( 'a' ) as $link ) {
			
			// Check that the link even points to our attachment datasources
			if (preg_match ( '/file\.php/', $link->getAttribute ( 'href' ) )) {
				...
				// Run the associated method to add the attachment to the page.
				...
				// This link isn't to /file.php, but the admin has asked us to check if it is a youtube link.
				// The overhead of checking strpos on every URL is less than the overhead of checking for a youtube ID
				if (strpos ( $link->getAttribute ( 'href' ), 'youtu' ) !== FALSE) {
	...
	/**
	 * Converts a YouTube link into YouTube player
	 *
	 * Fully loaded Links only, ie: <a href="youtube.com/v/12345">Link to youtube</a> only, not just a bare youtube URL.
	 *
	 * @param DOMDocument $doc        	
	 * @param DOMElement $link        	
	 */
	private function addYoutube(DOMDocument $doc, DOMElement $link) {
		...
		if ($youtube_id !== FALSE) {
			...
			$player->setAttribute ( 'width', '560' ); //TODO: Make admin adjustable? CSS?
	...
	/**
	 * Embeds embeddable videos into the DOM as players
	 *
	 * @param DOMDocument $doc        	
	 * @param DOMElement $link        	
	 */
	private function addVideo(DOMDocument $doc, DOMElement $link) {
		$video = $doc->createElement ( 'video' ); // Construct a <video> element. ;-)
		$video->setAttribute ( 'controls', 1 ); // Display the play button etc.
		$source = $doc->createElement ( 'source' ); // Set the source of the video as the file
	...
	/**
	 * Fetches an HTML attachment entirely, and injects it into entirely into the DOM
	 *
	 * @param DOMDocument $doc        	
	 * @param DOMElement $link        	
	 * 
	...
	private function addHTML(DOMDocument $doc, DOMElement $link) {
		try {
			...
			// Grab the data from the file, run it through osTicket's HTML filter, then load it into the new DOM
			@$html_document->loadHTML ( Format::sanitize ( $this->convertAttachmentUrlToFileContents ( $url ) ) );
			// Now we are able to grab every DOMElement from the new DOM and import them into our DOMDocument
			$node = $doc->importNode ( $html_document->documentElement, true );
			// Ensure it looks like the other attachments.
			$this->wrap ( $doc, $link, $node );
		} catch ( \Exception $e ) {
			// Likely, we'll get some form of "Wrong Document Exception".. particularly on very poorly formed HTML
	...
	/**
	 * Fetches a TEXT attachment entirely, and injects it into the DOM as <pre> 
	 *
	 * @param DOMDocument $doc        	
	 * @param DOMElement $link        	
	 */
	...
	/**
	 * Embeds an Office Document as an iframe pointing to a Google Docs Viewer
	 *
	 * @param DOMDocument $doc        	
	 * @param DOMElement $link        	
	 */
	 ...
	 * Just wanted a new DOMElement without having to copy & paste..
	 ...
	 *
	 * Looks painful, haven't tested the performance impact, however Admin can disable this.
	 *
	 * @param string $url        	
	 ...
	 * 
	 * Might have been originally written by Jay: https://davidwalsh.name/php-file-extension#comment-32903
	 * 
	 * Should probably just replace it with: pathinfo($string, PATHINFO_EXTENSION);
	 *
	 * @param DOMElement $link        	
	 * @return string
	 */
	...
	public static function isTicketsView() {
		...
		if (! isset ( $tickets_view )) {
			if (isset ( $_POST ) && count ( $_POST ) > 0) {
				...
				// ALSO: The HTML returned will not be a complete DOM, so we'll break it trying to change it at this time.
